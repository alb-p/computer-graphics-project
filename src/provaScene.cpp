...
struct CollectibleItem {
    ...
    bool isCollected;
    std::string name;
    CollectibleItem(bool isCollected, std::string name):isCollected(isCollected),name(name){};
};
...
class A04 : public BaseProject {
    ...
    float Yaw;
    glm::vec3 InitialPos;
    std::vector<std::string> landscape =  {"pavimento","apar"};

    std::vector<std::string> subject = {"c1"};
    CollectibleItem object = CollectibleItem(false,"objectToCollect");
...
/*        std::vector<Vertex> vertices = {
    ...
    glm::vec3 itemPosition =  glm::vec3(-15.0, 0.0, -15.0);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        if(object.isCollected == true && (count(landscape.begin(), landscape.end(), object.name)>0)){
            //landscape.pop_back();
        }
        if(object.isCollected && stopRendering){
            //SC.InstanceIds.erase("objectToCollect");
            landscape.push_back("apar3");
            stopRendering = false;
            
        }
        
        if(object.isCollected == false && !(count(landscape.begin(), landscape.end(), object.name)>0)){
            landscape.push_back(object.name);
        }
...
//        std::cout << Pos.x << ", " << Pos.y << ", " << Pos.z << "\n";
        ...
        if (!object.isCollected && CheckCollision(Pos, itemPosition, 3)) {
            std::cout << Pos.x << ", " << Pos.z << ", collected \n";
